...
class USearchViewController: HYBaseViewController {
    ...
    private lazy var searchBar: UITextField = {
        ...
        sr.delegate = self
        NotificationCenter.default.addObserver(self, selector: #selector(textFieldTextDidChange(noti:)), name: UITextField.textDidChangeNotification, object: sr)
        return sr
    }()
    ...
    private lazy var historyTableView: UITableView = {
        ...
        tw.register(cellType: HYBaseTableViewCell.self)
    ...
    lazy var searchTableView: UITableView = {
        ...
        sw.register(cellType: HYBaseTableViewCell.self)
        return sw
    }()
    ...
    private func searchRelative(_ text: String) {
        if text.count > 0 {
            ...
            currentRequest = ApiLoadingProvider.request(UApi.searchRelative(inputText: text), model: [SearchItemModel].self) { (returnData) in
...
extension USearchViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if tableView == historyTableView {
            ...
            head?.titleLabel.text = section == 0 ? "看看你搜过什么" : "大家都在搜"
