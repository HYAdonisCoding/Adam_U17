...
//  Adam_20190705_U17
//
//  Created by Adonis_HongYang on 2019/7/8.
    ...
    fileprivate func noticeBarProperties() -> UNoticeBarProperties {
        ...
        case .onNavigationBar:
            properties = UNoticeBarProperties(shadowOffsetY: 3,
                                              ...
                                              viewFrame: CGRect(origin: CGPoint.zero, size: CGSize(width: screenWidth, height: 44.0 + UIApplication.shared.statusBarFrame.height)))
        case .onStatusBar:
            properties = UNoticeBarProperties(shadowOffsetY: 2,
                                              fontSizeScaleFactor: 0.75,
                                              textFont: UIFont.systemFont(ofSize: 13),
                                              viewFrame: CGRect(origin: CGPoint.zero, size: CGSize(width: screenWidth, height: UIApplication.shared.statusBarFrame.height)))
        }
        return properties
    }
    
    fileprivate func noticesBarOriginY(superViewHeight: CGFloat, _ height: CGFloat) -> CGFloat {
...
public struct UNoticeBarConfig {
    ...
    public init(title: String? = nil,
                ...
                barStyle: UNoticeBarStyle = UNoticeBarStyle.onNavigationBar,
                animationType: UNoticeBarAnimationType = UNoticeBarAnimationType.top) {
        self.title = title
        self.image = image
...
open class UNoticeBar: UIView {
    ...
    private func configSubViews() {
        ...
        if let image = config.image, config.barStyle != .onStatusBar {
            ...
            let imageViewOriginY = config.barStyle.noticesBarOriginY(superViewHeight: frame.height, imageViewWidth)
            ...
            titleLabelOriginY = config.barStyle.noticesBarOriginY(superViewHeight: frame.height, titleLabelHeight)
        }
        
        _titleLabel?.frame = CGRect(x: titleLabelOriginX, y: titleLabelOriginY, width: titleLabelWidth, height: titleLabelHeight)
    }
    
    private func show(duration: TimeInterval, willShow: () -> Void, completed: ((_ ffinished: Bool) -> Void)?) {
